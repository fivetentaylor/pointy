...
func (h *Handler) computeAttrs(
	...
	if err := h.h.Handle(ctx, r); err != nil {
		return fmt.Sprintf("[prettylog] error when calling inner handler's Handle: %s", err)
	}

	var attrs map[string]any
	err := json.Unmarshal(h.b.Bytes(), &attrs)
	if err != nil {
		return fmt.Sprintf("[prettylog] error when unmarshaling inner handler's Handle result: %s", err)
	}

	var builder strings.Builder
	for k, v := range attrs {
		builder.WriteString(colorizer(darkGray, k+"="))
